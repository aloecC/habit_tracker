...
class HabitNiceTest(APITestCase):
    ...
    def setUp(self):
        ...
        self.action_2 = LikeAction.objects.create(
            name="Бегать по утрам", description="Test Description", owner=self.user
        )

        self.habit_nice = HabitNice.objects.create(
            like_action=self.action, location=self.location, user=self.user
        )

    def test_create_habit_nice(self):
        """Тест создания приятной привычки"""
        self.client.force_authenticate(user=self.user)
        data = {"like_action": f"{self.action.id}", "location": f"{self.location.id}"}
        response = self.client.post("/habits/habitnice/create/", data=data)
    ...
    def test_update_habit_nice(self):
        ...
        response = self.client.patch(
            f"/habits/habitnice/update/{self.habit_nice.id}/", data=data
        )
...
class HabitUsefulTest(APITestCase):
    ...
    def setUp(self):
        ...
        self.action = NeedAction.objects.create(
            name="Холодный душ", description="Test Description", owner=self.user
        )
        self.action_2 = NeedAction.objects.create(
            name="Бегать по утрам", description="Test Description", owner=self.user
        )
        ...
        self.habit_userful = HabitUseful.objects.create(
            need_action=self.action,
            location=self.location,
            reward=self.reward,
            user=self.user,
        )
        self.habit_userful_2 = HabitUseful.objects.create(
            need_action=self.action_2,
            location=self.location,
            reward=self.reward,
            user=self.user,
            is_public=True,
    ...
    def test_create_habit_userful(self):
        ...
        response = self.client.post("/habits/habituseful/create/", data=data)
    ...
    def test_update_habit_userful(self):
        ...
        response = self.client.patch(
            f"/habits/habituseful/update/{self.habit_userful.id}/", data=data
        )
    ...
    def test_delete_habit_userful(self):
        ...
        self.assertIsNotNone(
            HabitUseful.objects.filter(id=self.habit_userful.id).first()
        )
